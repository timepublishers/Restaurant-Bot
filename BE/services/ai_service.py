...
from database.tenant_db import TenantDatabase, MenuItem, Order, OrderItem, Session as ChatSession, Message, Settings, OrderStatus, PaymentStatus
    ...
    def list_menu(self, search: Optional[str] = None) -> str:
        ...
        try:
            ...
            for item in menu_items:
                result += f"- {item.name} (${float(item.price):.2f})"
                if item.description:
